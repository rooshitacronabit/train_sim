...
import { Model as SignalMainLunarRightModel } from "../models/signals/SignalMainLunarRightModel";
...
import { Model as SignalMainLunarThree } from "../models/signals/SignalMainLunarRightThreeModel";

import { Model as SignalCounter } from "../models/signals/SignalCounter";

import { signalData } from "../trackPoints";
...
export function Signal({
    ...
    (payload: IntersectionEnterPayload) => {
      ...
      if (name === startName) {
        if (!isFree) {
          ...
          setPopUpContent(
            signalData.filter((signal) => signal.label == label)[0]
          );
  ...
  useEffect(() => {
    ...
    if (green) {
      green.material =
        isFree && endSignal.includes(label)
          ? noStopOnMaterial
          : noStopOffMaterial;
    }
    ...
    switch (type) {
      ...
      case "signal-main-lunar-left":
      case "signal-main-lunar-right-three":
        if (label === "S-35" && currentLine == "3 line") break;
        if (label === "S-5" && currentLine == "4 line" || currentLine == "4-line")
          break;
        updateLunarLights();
        break;
